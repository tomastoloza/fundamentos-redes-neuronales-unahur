...
class ExploradorEspacioLatenteTipografia:
    def __init__(self, modelo_path=None, tamaño_imagen=32):
        self.procesador = ProcesadorDatosTipografia(tamaño_imagen)
        self.datos = self.procesador.obtener_datos_procesados()
        self.simbolos = self.procesador.simbolos
    ...
    def explorar_interactivo(self):
        ...
        for i, (x, y) in enumerate(self.latentes):
            simbolo = self.simbolos[i] if i < len(self.simbolos) else str(i)
            ax_latente.annotate(simbolo, (x, y), xytext=(3, 3), textcoords='offset points', fontsize=10)
        
        ax_latente.set_title('Espacio Latente 2D - Tipografía\n(Click para generar)')
        ...
        im_patron = ax_patron.imshow(np.zeros((self.tamaño_imagen, self.tamaño_imagen)), 
                                      cmap='gray_r', interpolation='nearest', vmin=0, vmax=1)
        ax_patron.set_title('Patrón Original')
        ax_patron.axis('off')
        
        im_reconstruido = ax_reconstruido.imshow(np.zeros((self.tamaño_imagen, self.tamaño_imagen)), 
                                                  cmap='gray_r', interpolation='nearest', vmin=0, vmax=1)
        ...
        def actualizar_patron(x, y):
            ...
            if distancias[idx_cercano] < 2.0:
                patron_original = self.datos[idx_cercano].reshape(self.tamaño_imagen, self.tamaño_imagen)
                im_patron.set_array(patron_original)
                simbolo = self.simbolos[idx_cercano] if idx_cercano < len(self.simbolos) else str(idx_cercano)
                ax_patron.set_title(f'Patrón Original: {simbolo}')
            else:
                im_patron.set_array(np.zeros((self.tamaño_imagen, self.tamaño_imagen)))
                ax_patron.set_title('Patrón Original (ninguno)')
            
            nuevo_patron = self.generar_desde_latente(np.array([[x, y]]))
            nuevo_patron_bin = (nuevo_patron > 0.5).astype(int).reshape(self.tamaño_imagen, self.tamaño_imagen)
        ...
        plt.suptitle('Explorador Interactivo del Espacio Latente - Tipografía\n'
                    'Click en el espacio latente o usa los sliders para explorar', fontsize=14)
        plt.tight_layout()
        plt.show()

def main():
    parser = argparse.ArgumentParser(description='Explorador interactivo del espacio latente para tipografía')
    parser.add_argument('modelo', type=str,
                       help='Nombre del modelo a cargar (ej: tp3_tipografia_lat2_ep500_lr0_001)')
    parser.add_argument('--tamaño', type=int, default=32,
                       help='Tamaño de imagen (default: 32)')
    
    args = parser.parse_args()

    explorador = ExploradorEspacioLatenteTipografia(args.modelo, args.tamaño)
